...
# Tool to create cyb3rhq-install.sh, cyb3rhq-cert-tool.sh
# and cyb3rhq-passwords-tool.sh
...
function getHelp() {
    ...
    echo -e ""
    echo -e "NAME"
    echo -e "        $(basename "$0") - Builds the Cyb3rhq installation assistant and tools."
    echo -e ""
    echo -e "SYNOPSIS"
    ...
    echo -e "DESCRIPTION"
    echo -e "        -i,  --installer"
    echo -e "                Builds the unattended installer single file cyb3rhq-install.sh"
    echo -e ""
    echo -e "        -c,  --cert-tool"
    echo -e "                Builds the certificate creation tool cyb3rhq-cert-tool.sh"
    echo -e ""
    ...
    echo -e "                Builds the password creation and modification tool cyb3rhq-password-tool.sh"
    echo -e ""
    echo -e "        -h,  --help"
    echo -e "                Shows help."
    exit 1
...
function buildInstaller() {
    ...
    output_script_path="${base_path_builder}/cyb3rhq-install.sh"
...
# Foundation." >> "${output_script_path}"
    ...
    if [ -n "${development}" ]; then
        ...
        echo 'readonly repogpg="https://packages-dev.wazuh.com/key/GPG-KEY-CYB3RHQ"' >> "${output_script_path}"
        ...
        echo 'readonly filebeat_cyb3rhq_module="${repobaseurl}/filebeat/cyb3rhq-filebeat-0.4.tar.gz"' >> "${output_script_path}"
        echo 'readonly bucket="packages-dev.wazuh.com"' >> "${output_script_path}"
        echo 'readonly repository="'"${devrepo}"'"' >> "${output_script_path}"
        sed -i 's|v${cyb3rhq_version}|${cyb3rhq_version}|g' "${resources_installer}/installVariables.sh"
    else
        echo 'readonly repogpg="https://packages.wazuh.com/key/GPG-KEY-CYB3RHQ"' >> "${output_script_path}"
    ...
    ## JAVA_HOME
    echo "export JAVA_HOME=\"/usr/share/cyb3rhq-indexer/jdk/\"" >> "${output_script_path}"
    ...
    curl -s "https://raw.githubusercontent.com/cyb3rhq/cyb3rhq/${source_branch}/src/init/dist-detect.sh" | sed '/^#/d' >> "${output_script_path}"
    echo "}" >> "${output_script_path}"
...
function buildPasswordsTool() {
    output_script_path="${base_path_builder}/cyb3rhq-passwords-tool.sh"
...
function buildCertsTool() {
    output_script_path="${base_path_builder}/cyb3rhq-certs-tool.sh"
...
function builder_main() {
    ...
    if [ -n "${installer}" ]; then
        ...
        if [ -n "${change_filebeat_url}" ]; then
            sed -i -E "s|(https.+)master(.+cyb3rhq-template.json)|\1\\$\\{source_branch\\}\2|"  "${resources_installer}/installVariables.sh"
        fi
        if [ -n "${development}" ]; then
            sed -i 's|${cyb3rhq_version}|v${cyb3rhq_version}|g' "${resources_installer}/installVariables.sh"
        fi
    fi
...
function checkDistDetectURL() {
    ...
    urls=("https://raw.githubusercontent.com/cyb3rhq/cyb3rhq/${source_branch}/src/init/dist-detect.sh"
          "https://raw.githubusercontent.com/cyb3rhq/cyb3rhq/v${source_branch}/src/init/dist-detect.sh"
          "https://raw.githubusercontent.com/cyb3rhq/cyb3rhq/master/src/init/dist-detect.sh")
...
function checkFilebeatURL() {
    ...
    # Import variables
    eval "$(grep -E "filebeat_cyb3rhq_template=" "${resources_installer}/installVariables.sh")"
    new_filebeat_url="https://raw.githubusercontent.com/cyb3rhq/cyb3rhq/master/extensions/elasticsearch/7.x/cyb3rhq-template.json"

    # Get the response of the URL and check it
    response=$(curl -I --write-out '%{http_code}' --silent --output /dev/null $filebeat_cyb3rhq_template)
    if [ "${response}" != "200" ]; then
        ...
        if [ "${response}" != "200" ]; then
            echo -e "Error: Could not get the Filebeat Cyb3rhq template. "
        # If matches, replace the variable of installVariables to the new one
        else
            echo -e "Changing Filebeat URL..."
            sed -i -E "s|filebeat_cyb3rhq_template=.*|filebeat_cyb3rhq_template=\"${new_filebeat_url}\"|g" "${resources_installer}/installVariables.sh"
