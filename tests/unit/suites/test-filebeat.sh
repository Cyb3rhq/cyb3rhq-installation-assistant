...
@setup-test {
    ...
    cyb3rhq_major="4.3"
    filebeat_cyb3rhq_template="https://raw.githubusercontent.com/cyb3rhq/cyb3rhq/${cyb3rhq_major}/extensions/elasticsearch/7.x/cyb3rhq-template.json"
    repobaseurl="https://packages.wazuh.com/4.x"
    filebeat_cyb3rhq_module="${repobaseurl}/filebeat/cyb3rhq-filebeat-0.1.tar.gz"
}
...
test-03-filebeat_copyCertificates-distributed-assert() {
    tar -xf /tmp/tarfile.tar -C /etc/filebeat/certs/ ./cyb3rhq1.pem
    mv /etc/filebeat/certs/cyb3rhq1.pem /etc/filebeat/certs/filebeat.pem
    tar -xf /tmp/tarfile.tar -C /etc/filebeat/certs/ ./cyb3rhq1-key.pem
    mv /etc/filebeat/certs/cyb3rhq1-key.pem /etc/filebeat/certs/filebeat-key.pem
    tar -xf /tmp/tarfile.tar -C /etc/filebeat/certs/ ./root-ca.pem
}
...
test-08-filebeat_configure-no-previous-variables() {
    ...
    filebeat_cyb3rhq_template=""
    filebeat_cyb3rhq_module=""
...
test-08-filebeat_configure-no-previous-variables-assert() {
    curl -so /etc/filebeat/cyb3rhq-template.json --max-time 300
...
test-09-filebeat_configure-one-elastic-node() {
    load-filebeat_configure
    @mocktrue curl -s ${filebeat_cyb3rhq_module} --max-time 300
    @mock tar -xvz -C /usr/share/filebeat/module
    @mocktrue echo admin
...
test-09-filebeat_configure-one-elastic-node-assert() {
    curl -so /etc/filebeat/cyb3rhq-template.json https://raw.githubusercontent.com/cyb3rhq/cyb3rhq/4.3/extensions/elasticsearch/7.x/cyb3rhq-template.json --max-time 300
    chmod go+r /etc/filebeat/cyb3rhq-template.json
    installCommon_getConfig filebeat/filebeat_distributed.yml /etc/filebeat/filebeat.yml
    echo "output.elasticsearch.hosts:" >> /etc/filebeat/filebeat.yml
